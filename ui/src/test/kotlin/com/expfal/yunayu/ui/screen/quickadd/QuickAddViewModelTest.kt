...
class QuickAddViewModelTest {
    ...
    @Test
    fun `setType income reloads suggestions with income type`() = runTest {
        val tagRepo = FakeTagRepository().apply { recentTags = listOf(tag(1L, "学习")) }
        val viewModel = viewModel(tagRepo, FakeTransactionRepository())

        assertEquals(listOf(TransactionType.EXPENSE), tagRepo.recentTypeCalls)

        viewModel.setType(TransactionType.INCOME)
        runCurrent()

        assertEquals(TransactionType.INCOME, viewModel.uiState.value.transactionType)
        assertEquals(listOf(TransactionType.EXPENSE, TransactionType.INCOME), tagRepo.recentTypeCalls)
    }

    @Test
    fun `setType is ignored and does not refresh while saving`() = runTest {
        val gate = CompletableDeferred<Long>()
        val txRepo = FakeTransactionRepository().apply { addGate = gate }
        val tagRepo = FakeTagRepository().apply { recentTags = listOf(tag(1L, "学习")) }
        val viewModel = viewModel(tagRepo, txRepo)
        ...
        viewModel.onSave()
        assertTrue(viewModel.uiState.value.saving)
        val callsBefore = tagRepo.recentTypeCalls.size

        viewModel.setType(TransactionType.INCOME)
        assertEquals(TransactionType.EXPENSE, viewModel.uiState.value.transactionType)
        assertEquals(callsBefore, tagRepo.recentTypeCalls.size)
    ...
    @Test
    fun `setType is ignored while nlParsing`() = runTest {
        val gate = CompletableDeferred<String>()
        val nlParser = FakeNlParser().apply { generateGate = gate }
        val viewModel = viewModel(FakeTagRepository(), FakeTransactionRepository(), nlParser)

        viewModel.setNlMode(true)
        viewModel.onNlInputChange("午饭20")
        viewModel.onParseNl()
        assertTrue(viewModel.uiState.value.nlParsing)

        viewModel.setType(TransactionType.INCOME)
        assertEquals(TransactionType.EXPENSE, viewModel.uiState.value.transactionType)

        gate.complete("{\"amount\":\"20\"}")
        runCurrent()
        assertFalse(viewModel.uiState.value.nlParsing)
    }

    @Test
    fun `setType is ignored and does not refresh while confirmRequested`() = runTest {
        val tagRepo = FakeTagRepository().apply { recentTags = listOf(tag(1L, "学习")) }
        val viewModel = viewModel(tagRepo, FakeTransactionRepository())
    ...
    @Test
    fun `refreshSuggestedTags in NL mode does not touch selection`() = runTest {
        val tagRepo = FakeTagRepository().apply {
            recentTags = listOf(tag(1L, "学习"), tag(2L, "社交"))
        }
        val viewModel = viewModel(tagRepo, FakeTransactionRepository())
    ...
    @Test
    fun `digital mode saves with null tagId when no tag selected`() = runTest {
        val txRepo = FakeTransactionRepository()
        val tagRepo = FakeTagRepository().apply {
            recentTags = emptyList()
            rootTags = emptyList()
    ...
    @Test
    fun `NL mode saves with null tagId when no tag matched`() = runTest {
        val txRepo = FakeTransactionRepository()
        val tagRepo = FakeTagRepository().apply { rootTags = listOf(tag(1L, "学习")) }
        val nlParser = FakeNlParser().apply {
            generateResult = "{\"amount\":\"20\",\"note\":\"午饭\"}"
        }
        val viewModel = viewModel(tagRepo, txRepo, nlParser)
    ...
    @Test
    fun `createSubTag in digital mode selects new tag and refreshes by type`() = runTest {
        val tagRepo = FakeTagRepository().apply {
            rootTags = listOf(tag(1L, "学习"), tag(2L, "生活"))
            recentTags = listOf(tag(1L, "学习"))
            addSubTagResult = 99L
    ...
    @Test
    fun `createSubTag in NL mode syncs nlTagId and selectedTagId`() = runTest {
        val tagRepo = FakeTagRepository().apply {
            rootTags = listOf(tag(1L, "学习"), tag(2L, "生活"))
            addSubTagResult = 99L
        }
        ...
        assertEquals(99L, viewModel.uiState.value.selectedTagId)
        assertEquals(99L, viewModel.uiState.value.nlTagId)
    }
