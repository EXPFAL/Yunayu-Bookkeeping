...
- 饼状图：Canvas 自绘（不引第三方库）、固定色板、Top8 +「其他」桶闭合 360°；扇区/图例可点选高亮，圆心显示金额占比，下方详情含笔数/日均/最近流水，「查看流水」再下钻收支管理
- 验收要点：「本周」选项可切换；周报数据正确；饼状图显示 Top8 分类 + 其他；跨年周正确处理；点选分类可见详情
...
- 验收要点：无 API 仍有概览/洞察；记账后报告 STALE；打开报告可见本期；点选分类看详情后再「查看流水」跳转过滤列表

### 36.1 报告饼图交互与分类详情
- 痛点：饼图仅展示；Top5 信息偏少；缺少笔数/日均/流水预览
- 方案：交互环形图（命中选中 + 圆心文案）；Top8 +「其他」可展开剩余分类；`LoadCategoryExpenseDetailUseCase` 拉笔数与最近 5 笔
- 验收要点：列表与饼图选中同步；「其他」可展开；查看流水时间窗正确
...
- 饼图其他桶：total - topSum 确保闭合 360°，避免浮点误差导致缺口
...
### 40. AI 结合备注 + 记一笔即时分类建议
- 痛点：报告 AI 只吃汇总数字，难理解消费场景；记一笔有备注但未选分类时不知挂哪一类
- 方案：
  - 记一笔：备注非空且未选手动分类时，debounce 300ms 调用 `SuggestTagsFromNoteUseCase`，给出 1～3 个已有标签建议芯片（可一键选中）；本期只 ATTACH 已有标签
  - 报告 AI：按 Top 分类各抽最多 3 条非空备注（金额降序），合计 ≤800 字写入 prompt「各类代表备注」
- 口径：保存仍允许无标签（未分类）；API 不可用时无建议/无备注段，不挡主流程；整理链路行为不变；账户「未指定」不改
- 验收要点：有备注无分类时出现可点建议芯片；点选后随保存写入；生成报告时 AI 输入含代表备注；无备注时不报错
